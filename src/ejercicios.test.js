...
const { nuevaString, nuevoNum, nuevoBool, nuevaResta, nuevaMultiplicacion, nuevoModulo, devolverString, tienenMismaLongitud, sonIguales, menosQueNoventa, mayorQueCincuenta, suma, resta, divide, multiplica, obtenerResto, esPar, esImpar, elevarAlCuadrado, elevarAlCubo, elevar, redondearNumero, redondearHaciaArriba, numeroRandom, esPositivo, agregarSimboloExclamacion, combinarNombres, obtenerSaludo, obtenerAreaRectangulo, retornarPerimetro, areaDelTriangulo, deEuroAdolar, esVocal } = require("./ejercicios.js");
...
describe("agregarSimboloExclamacion(str)", function () {
	it("Deberia agregar un signo de exclamacion al final del string", function () {
		...
		expect(agregarSimboloExclamacion("Soy Hola")).toBe("Soy Hola!");
	});
});
...
describe("combinarNombres(firstName, lastName)", function () {
	it("Deberia devolver los strings combinados con un espacio en el medio", function () {
		...
		expect(combinarNombres("Soy", "Hola")).toBe("Soy Hola");
	});
});
